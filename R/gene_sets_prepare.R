...
gene_sets_prepare <- function(path_to_db_file, cell_type){
  ...
  gs = lapply(seq_len(nrow(cell_markers)), function(j) gsub(" ","",unlist(strsplit(toString(cell_markers$geneSymbolmore1[j]),",")))); names(gs) = cell_markers$cellName
  gs2 = lapply(seq_len(nrow(cell_markers)), function(j) gsub(" ","",unlist(strsplit(toString(cell_markers$geneSymbolmore2[j]),",")))); names(gs2) = cell_markers$cellName
